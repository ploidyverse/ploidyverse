#' Update ploidyverse packages
#'
#' This will check to see if all ploidyverse packages (and optionally, their
#' dependencies) are up-to-date, and will install after an interactive
#' confirmation.
#'
#' @param recursive If \code{TRUE}, will also check all dependencies of
#'   ploidyverse packages.
#' @export
#' @examples
#' \dontrun{
#' ploidyverse_update()
#' }
ploidyverse_update <- function(recursive = FALSE) {

  deps <- ploidyverse_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All ploidyverse packages up-to-date")
    return(invisible())
  }
...
#' List all ploidyverse dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   ploidyverse packages.
#' @export
ploidyverse_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("ploidyverse", pkgs, recursive = recursive)
